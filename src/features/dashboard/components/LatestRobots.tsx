...
function SkeletonLatestRobots() {
  return (
    <div className="rounded-[18px] border border-brand-neon/22 overflow-hidden bg-brand-bg/18">
      {[...Array(5)].map((_, i) => (
        <div
          key={i}
          className={`flex items-center justify-between gap-3 px-3.5 py-3${i > 0 ? ' border-t border-brand-neon/18' : ''}`}
        >
          <div className="flex flex-col gap-1.5 min-w-0 flex-1">
            <Skeleton variant="text" className="w-32 h-4" />
            <Skeleton variant="text" className="w-48 h-3" />
          </div>
          <Skeleton className="w-12 h-8 rounded-xl shrink-0" />
        </div>
      ))}
    </div>
  );
...
export function LatestRobots() {
  const { data: robots, isLoading, error } = useQuery<LatestRobot[]>({
  ...
  return (
    <>
      <AnimatedList className="rounded-[18px] border border-brand-neon/22 overflow-hidden bg-brand-bg/18">
        {robots.map((robot, i: number) => {
          const { nombre, categoria, equipo } = getRobotDisplay(robot);
          const sub = [robot.robot_id, categoria, equipo].filter(Boolean).join(" · ");
          return (
            <AnimatedListItem key={robot.robot_id}>
              <Link
      ...
      <div className="text-brand-muted/45 text-[11px] leading-relaxed mt-3">
         Tip: El QR abre directo en <b>/robots/ROBOT_ID</b> para mostrar la ficha del robot.
      </div>
    </>
  );
}
